...
def plot_top_words_vs_avg_vote(df, n=40):
    ...
    counter = Counter(all_words)
    _words = [word for word, _ in counter.most_common(n)]
    # X = word, y = average vote
    avg_votes = []
    for word in _words:
        avg_vote = df[df['clean_notes'].str.contains(r'\b' + re.escape(word) + r'\b', na=False)]['vote'].mean()
        avg_votes.append(avg_vote)
    plt.figure(figsize=(12, 6))
    sns.barplot(x=_words, y=avg_votes, palette='viridis')
    plt.title(f'Top {n} Words vs Average Vote')
    plt.xlabel('Words')
